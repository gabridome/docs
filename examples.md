...
From Bitcoin Core (0.17), it is possible to scan the UTXO set to retrieve the transaction outputs relevant for a set of public keys.
It is possible to obtain a derived Public extended Key further derivable and extract all the relative UTXOs, according to 
specific *[Output Descriptors]*[1].
...
```
and to use it in Bitcoin Core (0.17) deriving it further:
* from 0/* for normal receiving addresses
* from 1/* for change internal addresses 

Now we can extract the relevant UTXOs using the [descriptors][1].
...
It is necessary to extract the extended public subkey until the last hardened level (m/84h/0h/0h)
